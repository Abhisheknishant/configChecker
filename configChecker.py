...
class ConfigChecker():
    ...
    def set_expectation(self, section, key, data_type, default, message = None):
        ...
        if data_type not in [bool,float,str,int]:
            log.warning("Trying to add expection with not allowed data type [{}]. Allowed types = [bool, int, str, float]".format(data_type))
            return False
        ...
        log.debug("Added new expectation with Section: [{}], Key [{}], DataType [{}], Default [{}]".format(section,key,data_type,default))
        return True

    def __log_wrong_expectation_data_type(self,section,key,data_type,default):
        log.warning("Trying to set expectation Section: [{}], Key [{}], Default [{}] with wrong data type. (Type = [{}])".format(section,key,default,data_type))
    ...
    def __log_value_update(self,section,key,value,position,success):
        if success:
            ...
            log.warning("Cannot update the value of Section [{}], Key [{}], from [{}] to [{}], wrong type (type = [{}])".format(section,
                key,self.__expectations[position]['value'],value,self.__expectations[position]['data_type']))
